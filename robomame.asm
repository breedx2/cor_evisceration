...
family_member_list_entry_pointer EQU $9849     ; not same as $981F. This field is a pointer to an entry in a quick lookup list used by hulks and brains to find family members to target.
...
????   EQU $984F                               ; TODO: see $2717
????   EQU $9850
credits EQU $9851
???? EQU $9859                                 ; #$7F = player died.
...
$BE65 - used by tank shell creation routine. I haven't reverse engineered this part entirely yet so purpose is unknown.
...
; CMOS settings
extra_man_every EQU $CC00                                ; in $CC01: 20 = every 20000, 25 = every 25000, 30 = every 30000, 50 = every 50000
fancy_attract_mode_on_off EQU $CC13                      ; 1 = on, 0 = off
turns_per_player EQU $CC02
pricing_selection EQU $CC05                              ; 0 - 9. 9 = free play
     left_slot_units EQU $CC07
     centre_slot_units_digit_1 EQU $CC08
     centre_slot_units_digit_2 EQU $CC09
...
difficulty_of_play EQU $CC14                            ; in $CC15: 0 - 10.   0 = extra liberal, 5 = recommended, 10 = extra conservative
letters_for_highest_score_name EQU $CC17


credits_cmos EQU $CD00
top_score EQU $CD32
...
0036: 86 08       LDA   #$08                ; game cycles before calling routine below
...
004B: 6F 4B       CLR   $000B,U             ; set index to 0, to start the animation off again
004D: 20 F1       BRA   $0040
...
; LDY [$09,U] gets a pointer to the *next* object in the object metadata linked list.
...
; this code makes the hulk move around the object pointed to in register Y - its "target".
; think of it like this - the hulk identifies a 2D Point (X,Y) to move to, where
; point.X is within the range ((current target object's X coordinate - 15) ... current target's X coordinate + 16)
; point.Y is within in the range ((current target object's Y coordinate - 15) ... current target's Y coordinate +16)
;
...
0247: 27 14       BEQ   $025D                ; if no family members left (all dead or rescued) then goto $025D
...
02B8: CE 03 30    LDU   #$0330               ; Load U with address of routine that reduces mikey count
...
02C3: CE 03 35    LDU   #$0335               ; Load U with address of routine that reduces mommy count
...
02CE: CE 03 3A    LDU   #$033A               ; load U with address of routine that reduces daddy count
...
08A5: 88 00 02 20 22 00 00 00 00 00 00 00 00 02 55 00
...
0925: 87 88 00 00 00 02 20 00 00 00 00 00 00 00 00 55
...
0965: 00 70 87 88 00 00 22 02 20 00 00 00 00 00 00 00
...
1146: 14 F2 18 D2 18 E6 D8 01 08 18 00
1151: D1 01 08 08 00
...
116D: 27 3E       BEQ   $11AD                ; if spheroid count ==0, goto $11AD
...
11E5: AE 47       LDX   $0007,U              ; load X with pointer to object
...
121C: 8E 11 E5    LDX   #$11E5               ; address of function to call next
...
; at this point B is anc X coordinate that the enforcer is considering moving to.
13C2: C1 07       CMPB  #$07                 ; at top-most of playfield?
...
; Find the nearest family member to program. Now here's where the infamous "brains all go for the same family member"
...
;
; When $B354 *is* populated with a pointer to a family member, guess what family member gets the slot? That's right - Mikey!
...
1CD5: CC 21 41    LDD   #$2141               ; animation frame pointer of brain facing left
1CD8: 20 0D       BRA   $1CE7
...
1CE4: CC 21 4D    LDD   #$214D               ; animation frame pointer of brain facing right, programming human
1CE7: ED 02       STD   $0002,X              ; save pointer to current animation frame metadata
...
1D4C: 8E 1D 52    LDX   #$1D52               ; address of routine to call next
...
1D74: 8E 1D 7A    LDX   #$1D7A               ; address of function to call next
...
;
; This function here computes two variables: X Distance and Y Distance.
...
2141: 07 10               ; width: 7 bytes (14 pixels, remember 2 pixels per byte), height: 10 (16 decimal)  pixels
2143: 21 71               ; pointer to animation frame metadata for walking left frame 1

; brain walking left #2
2145: 07 10               ; width: 7 bytes (14 pixels), height: 10 (16 decimal)  pixels
2147: 21 E1               ; pointer to animation frame metadata for walking left frame 2
...
270D: 26 02       BNE   $2711
270F: 97 51       STA   $51                  ; store number of credits
2711: 91 51       CMPA  $51
2713: 22 E4       BHI   $26F9
...
2719: 0F 50       CLR   $50
271B: C6 08       LDB   #$08
271D: BD D0 BA    JSR   $D0BA                ; JMP $D65B -
...
2775: 97 48       STA   $48                  ; used in the collision detection function. I thought it was a player hit flag, but it's not.
...
27E2: BD D0 24    JSR   $D024                ; JMP $D89E
27E5: 0F F2       CLR   $F2
27E7: 96 48       LDA   $48
27E9: 26 05       BNE   $27F0
...
2867: 41 40       ; pointer to function, which is a JMP $459B
2869: 86 96       LDA   #$96                 ; delay before calling function
286B: 8E 28 74    LDX   #$2874               ; address of function to call (brain wave related)
...
2945: 35 02       PULS  A
2947: 8D 1C       BSR   $2965
2949: 86 01       LDA   #$01                 ; delay
294B: 8E 29 0D    LDX   #$290D               ; address of function
294E: 7E D0 66    JMP   $D066                ; JMP $D1E3 - allocate task

2951: 35 02       PULS  A
2953: 86 02       LDA   #$02                 ; delay
2955: 8E 29 5B    LDX   #$295B               ; address of function
2958: 7E D0 66    JMP   $D066                ; JMP $D1E3 - allocate task

295B: 8D 26       BSR   $2983
295D: 86 0A       LDA   #$0A                 ; delay
295F: 8E 28 74    LDX   #$2874               ; address of function
...
2A2E: 80 0A       SUBA  #$0A                 ; else subtract 10 from the index until we have a number from 0-9.
2A30: 20 F8       BRA   $2A2A
...
;
; Next bytes are 4,6, and then 0 which flags "start the animation from the beginning"
...
3101: BD D0 24    JSR   $D024                ; JMP $D89E
...
31CB: 84 F0       ANDA  #$F0                ; mask off top 4 bits (the fire button bits in A)
31CD: E6 47       LDB   $0007,U             ; U is set to AA85 at this point.
...
341A: 86 01       LDA   #$01
341C: 8E 33 FF    LDX   #$33FF               ; address of function to call (this one)
...
343D: 86 01       LDA   #$01
343F: 8E 34 22    LDX   #$3422               ; address of function to call (this one)
...
3463: 86 01       LDA   #$01
3465: 8E 34 45    LDX   #$3445               ; address of function to call (this one)
...
3856: 00 09 90 00 00 FF FF FF FF FF FF FF FF FF FF FF
...
39F3: 22 08       BHI   $39FD                ; no carry? if so, go to $39FD
...
3AFF: 8E 3A E0    LDX   #$3AE0               ; address of routine to call next (this one!)
...
4236: 8D 37       BSR   $426F
4238: 25 27       BCS   $4261                ; if failed, goto $4261
...
; B = number of bytes to clear (zero)
...
; X = pointer to animation frame metadata
;
; Returns: carry flag set or cleared because ???
;
...
4285: 1A 01       ORCC  #$01                ; set carry flag
...
45B6: ED 88 14    STD   $14,X                ; set previous animation frame metadata pointer (previous = current)
...
45D9: ED 02       STD   $0002,X              ; set animation frame metadata pointer
45DB: AE 84       LDX   ,X
45DD: 26 D5       BNE   $45B4
...
45FB: ED 02       STD   $0002,X              ; set current animation frame metadata pointer (current = previous)
45FD: AE 84       LDX   ,X
45FF: 26 F7       BNE   $45F8
...
4611: C6 12       LDB   #$12
4613: BD D0 06    JSR   $D006                ; JMP $D3B6
...
4626: C6 12       LDB   #$12
4628: BD D0 06    JSR   $D006                ; JMP $D3B6
462B: 6A 47       DEC   $0007,U
462D: 10 27 8A 32 LBEQ  $D063                ; JMP $D1F3 - free object metadata and process next task
4631: 86 02       LDA   #$02
...
464B: EE 22       LDU   $0002,Y              ; get actual image data pointer into U
...
;
; X = enemy to explode
; A = value determining the direction the enemy's top half must explode  ($FF = top half of explosion moves up and left, 1= top half of explosion moves up and right)
; B = value determining the direction the enemy's bottom half must explode ($FF = top half of explosion moves down and left, 1 = bottom half of explosion moves down and right)
;
...
; U = blitter flags
; $BA = offset to add to D after each blit, to space out the explosion segments
...
4A5F: 80 10       SUBA  #$10                 ; A -= 16. This works nicely if A is 0..16.
...
4C43: 81 14       CMPA  #$14                 ; compare number of tanks to #$14  (20 dec.)
...
4C69: 8E 4C 2E    LDX   #$4C2E              ; address of function to call next
...
4D13: 34 02       PSHS  A
4D15: 27 4A       BEQ   $4D61               ; if count ==0, no tanks on this wave, goto $4D61
...
4D8D: 85 7F       BITA  #$7F
4D8F: 27 08       BEQ   $4D99
4D91: 86 0F       LDA   #$0F                ; delay before calling function
...
;
; This piece of code animates the tank. If the tank is moving to the LEFT, the tank animation runs backwards,
...
; make tank animation play "in reverse""
...
4E81: C4 1F       ANDB  #$1F                 ; mask in bits 0..5, giving a number from 0..31
...
4EC8: 58          ASLB                       ; next 4 instructions move
4EC9: 49          ROLA
4ECA: 58          ASLB
4ECB: 49          ROLA
4ECC: 58          ASLB
...
5A17: A6 E4       LDA   ,S
5A19: 84 0F       ANDA  #$0F                 ; mask in right nibble of colour value
...
5A64: 86 05       LDA   #$05                ; width of 1 byte (= 2 pixels), blitter requires a value xor'd with 4
5A66: C8 04       EORB  #$04
...
5A7F: 5C          INCB
5A80: 1F 01       TFR   D,X                  ; X now is a blit destination. The horizontal line will be drawn from this address
...
5A93: C8 04       EORB  #$04                ; XOR height with 4 as the blitter chip needs this
5A95: F7 CA 06    STB   blitter_width       ; set width
5A98: 86 05       LDA   #$05                ; width of 1 byte, blitter requires a value xor'd with 4
...
5C23: 26 07       BNE   $5C2C               ; if the players laser is moving on the horizontal axis, goto $5C2C
...
5E76: BD D0 24    JSR   $D024               ; JMP $D89E
...
5EA6: BD D0 24    JSR   $D024               ; JMP $D89E
5EA9: DE 15       LDU   $15
...
604C: 2F 01       BLE   $604F               ; if 0 | (N xor V) == 1 goto $604C
604E: 50          NEGB
...
6147: 0F D0       CLR   $D0                 ; clear "blitter must do pixel shift" flag -see $6002 for more info
6149: 34 66       PSHS  U,Y,B,A
614B: C6 07       LDB   #$07                ; large font size indicator


614D: D7 D2       STB   $D2                 ; set font-size flag (see $6190 for how this affects what size of font is rendered)
614F: 0F D7       CLR   $D7
6151: 34 42       PSHS  U,A
...
61C0: 62 58
61C2: 62 6B
61C4: 62 0F                                  ; PRINT_AT_SPECIFIC_POSITION
61C6: 62 70
61C8: 61 CE
61CA: 62 3F
61CC: 62 5D
...
;
; Move relative of current text position.
;
; X = current text position (screen address where top left of character will be blitted)
;
...
61FD: 96 D0       LDA   $D0                  ; are we blitting on an odd pixel?
61FF: 81 01       CMPA  #$01                 ; if 1, then yes we are blitting on an odd pixel
6201: 27 06       BEQ   $6209                ; if yes, goto $6209, which will set A to 0, meaning "blit on even pixel"
...
6935 - 6EFF : reserved for text.
 ...
 
6CD1: 04 - change text colour to next byte
6CD2: 99 - colour
6CD3: instruction - JSR to $6275
6CD4
...
70A2: 1E 12       EXG   X,Y
70A4: BD D0 A5    JSR   $D0A5               ; JMP $D523 - pack 2 bytes at X into B as BCD
70A7: C1 FF       CMPB  #$FF
70A9: 26 02       BNE   $70AD
70AB: C6 EE       LDB   #$EE
70AD: 1E 12       EXG   X,Y
...
7142: 2B 22       BMI   $7166               ; Fire down (bit 7) is set
...
7182: 1F 21       TFR   Y,X
7184: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1
...
71A8: 1F 21       TFR   Y,X
71AA: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1
71AD: 4D          TSTA
71AE: 27 F4       BEQ   $71A4
...
71F0: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1
71F3: 4D          TSTA
71F4: 27 04       BEQ   $71FA
71F6: 1A 01       ORCC  #$01
...
7450: 1F 21       TFR   Y,X
7452: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1
7455: 48          ASLA
7456: 34 02       PSHS  A
7458: 48          ASLA
7459: AB E0       ADDA  ,S+
...
74B2: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1
74B5: A1 E0       CMPA  ,S+
74B7: 39          RTS
...
77E4: BD D0 12    JSR   $D012                ; JMP CLR_SCREEN1
...
;
; This is the entry point to the "ROBOTRON: 2084 - SAVE THE LAST HUMAN FAMILY" attract screen.
;
;
;
;
...
79B8: BD D0 24    JSR   $D024               ; JMP $D89E
...
;
79D4: 8D D9       BSR   $79AF
79D6: 8E 7F A3    LDX   #$7FA3              ; pointer to INSPIRED BY HIS NEVER ENDING QUEST FOR PROGRESS [..] text
...
; this function prints the message "INSPIRED BY HIS NEVER ENDING QUEST FOR PROGRESS [..]" character by character
79DD: AE 47       LDX   $0007,U             ; get pointer to current character to print
79DF: A6 80       LDA   ,X+                 ; read byte (which might be a character to print)
...
79EA: 48          ASLA                      ; multiply A * 2, to give offset into table from $7A08
79EB: AD 96       JSR   [A,X]               ; call function in table
79ED: 20 EE       BRA   $79DD               ; read next character or instruction

79EF: 81 5F       CMPA  #$5F
79F1: 25 06       BCS   $79F9
; if we get here, A is more than #$5F. Its not a character to print, but a delay to "sleep" for before printing the next character.
...
7A60: 86 14       LDA   #$14
7A62: C6 D8       LDB   #$D8
7A64: 1F 01       TFR   D,X                 ; X = $14D8, blitter dest
...
;
; TODO: this code is part of the "Robotron 2084: Save the last human family" attract page.
;


7B1E: BD D0 6F    JSR   $D06F                ; JMP $D2FD - call RESERVE_OBJECT_FOR_SPHEROID_ENFORCER_QUARK_SPARK_SHELL
; X = pointer to object
7B21: AF 4B       STX   $000B,U              ; save pointer to object
...
7FC3: GQUEST FOR PROGRESS<IN *2084
7FE3: ] MAN PERFECTS THE *ROBOTRONS?
8003:  A ROBOT SPECIES SO ADVANCED
8023:  THATMAN IS INFERIOR TO HIS OWN
...
80C3: MUST BE DESTROYED=p6@]7YOU A
...
8183:  LAST HUMAN FAMILY?w sXt
81A3: Xup~pX 3THE FORCE OF GRO
81C3: UND ROVINGUNIT NETWORK TERMINAT
81E3: OR *tv[GRUNT\:ROBOTRONS3 SEE
8203: K TODESTROY YOU=X U!THE*
8223: w HULK ROBOTRONS USEEK OUTAND
...
8303: TRONS THAT POSSESSTHE POWER T
8323: O REPROGRAMHUMANS INTO SINISTER
8343:  *PROGS=s,| Ld  ~@]AS
8363: YOU STRUGGLE TO SAVEHUMANITY< B
...
; I have a suspicion this is sound related code. This routine is called from events that create sounds such as lasers firing, player death.
;
...
D405: 85 40       BITA  #$40                   ; read "slam door tilt" bit
D407: 27 04       BEQ   $D40D
D409: 86 3C       LDA   #$3C
D40B: 97 4B       STA   $4B
D40D: 96 4B       LDA   $4B
...
D591: 8E CC 0C    LDX   #$CC0C               ; address of units_required_for_credit
...
D59C: 34 02       PSHS  A
D59E: D7 50       STB   $50
D5A0: 8E CC 0E    LDX   #$CC0E               ; address of units_required_for_bonus_credit
...
D688: 19          DAA
D689: 30 1A       LEAX  -$6,X
D68B: BD D0 AB    JSR   $D0AB               ; JMP STA_NIB_X
D68E: 35 04       PULS  B
D690: 35 02       PULS  A
D692: BD D0 B1    JSR   $D0B1               ; JMP $D537 - write A and B to X
D695: 35 02       PULS  A
...
D6AC: 34 04       PSHS  B
D6AE: 1F 89       TFR   A,B
D6B0: 8D 1B       BSR   $D6CD                ; call random number generator
D6B2: 3D          MUL
...
; X = pointer to linked list of objects to compare object ONE against.
...
D7CD: DD 7E       STD   $7E                  ; store in $7E. Think of $7C to $7F defining the rectangular area that object ONE occupies now.
D7CF: 20 17       BRA   $D7E8
...
D7F4: 10 AE 88 16 LDY   $16,X               ; has collision detection animation frame metadata been supplied for this object?
D7F8: 26 03       BNE   $D7FD               ; yes, so use the width and height in the metadata, goto $D7FD
D7FA: 10 AE 02    LDY   $0002,X             ; get pointer to animation frame metadata of object that we collided with
D7FD: A3 A4       SUBD  ,Y                  ; subtract width & height of object that might have collided, from its blitter destination
...
D817: 90 7C       SUBA  $7C                 ; A (the X component of blitter destination) -= left X coordinate of object ONE, to give horizontal distance in pixels div 2
...
D991: D9 97       ; address of data to pass to function
D993: 98 0C       ; second parameter
D995: 00 02       ; third parameter (discarded)

; This is data used by function at $D9E8
D997: C0 C0       SUBB  #$C0
D999: D0 E0       SUBB  $E0
...
D9B3: 98 0E       ; second parameter
D9B5: 00 01       ; third parameter (discarded)

; Data pointed to by $D9B1
D9B7: C0 C1       SUBB  #$C1
D9B9: C2 C3       SBCB  #$C3
D9BB: C4 C5       ANDB  #$C5
D9BD: C6 C7       LDB   #$C7
...
D9D5: D9 DB       ; parameter 1: address of data to pass to function
D9D7: 98 0F       ; second parameter
D9D9: 00 06       ; third parameter (discarded)

; Data pointed to by $D9D5
D9DB: 07 07       ASR   $07
D9DD: 2F 00       BLE   $D9DF

; Called by $D89E
D9DF: BD D9 E8    JSR   $D9E8
D9E2: DA 2C       ; parameter 1: address of data to pass to function
D9E4: 98 0D       ; second parameter
D9E6: 00 02       ; third parameter  (discarded)



;
; This method expects stack pointer to point to 3 parameters, which are a word size each (meaning, 6 bytes in total)
; Parameter 1: a pointer to ???
; Parameter 2: read, but never used. Hence why I use the term (discarded) above.
;
...
DA00: 27 F4       BEQ   $D9F6                ; if 0, then goto $D9F6, which resets the index into data
DA02: A7 D8 0B    STA   [$0B,U]
DA05: A6 4A       LDA   $000A,U              ; delay
...
DA22: 97 0A       STA   $0A                  ; store in #$980A
DA24: 86 06       LDA   #$06                 ; delay
...
;
; D = blitter destination
; Y = pointer to image struct (first 2 bytes are w & h, next 2 are pointer to blitter source)
;
...
;
; D = blitter destination
; Y = pointer to animation frame metadata (first 2 bytes are width & height, next 2 are pointer to blitter source)
;
...
DB9E: 0C F0       INC   $F0
DBA0: 44          LSRA                      ; divide number of trailing zeros by 2 - will also set carry if number of trailing zeros is odd number
DBA1: 34 02       PSHS  A                   ; save on stack
DBA3: 86 00       LDA   #$00
DBA5: 24 08       BCC   $DBAF
DBA7: 58          ASLB                      ; move 4 most significant bits of B into 4 least significant bits of A
DBA8: 49          ROLA
DBA9: 58          ASLB
DBAA: 49          ROLA
DBAB: 58          ASLB
...
DBAD: 58          ASLB
DBAE: 49          ROLA
DBAF: BD D0 45    JSR   $D045               ; JMP $D699 - get addr of current player game state into X. In this case X = pointer to score
DBB2: DD 2B       STD   $2B
DBB4: C6 03       LDB   #$03
DBB6: E0 E0       SUBB  ,S+                 ; subtract index on stack from B. B is now the index into the first digit to update.
DBB8: A6 85       LDA   B,X                 ; A = *(X + B)
DBBA: 9B 2C       ADDA  $2C
DBBC: 19          DAA                       ; remember, we're working with BCD here, so have to use Decimal Adjust A
DBBD: A7 85       STA   B,X                 ; update score digit at (X+B)
DBBF: 5A          DECB                      ; point to previous digit (ie: if we're at the tens digit, goto the hundreds digit, if we're at the hundreds digit go to the thousands and so on)
DBC0: 2B 0E       BMI   $DBD0               ; if b==-1 then goto $DBD0
DBC2: A6 85       LDA   B,X                 ; A = *(X+B)
DBC4: 99 2B       ADCA  $2B                 ; if we had a carry from the last add, then be sure to take that into account
DBC6: 19          DAA                       ; and again, ensure that we have valid BCD digits
DBC7: A7 85       STA   B,X                 ; update score digit at (X+B)
DBC9: 86 00       LDA   #$00                ; OK, we set the value to carry over to zero. So only carry flag will be added to remaining digits
DBCB: 97 2B       STA   $2B
DBCD: 5A          DECB                      ; point to previous digit (ie: if we're at the tens digit, goto the hundreds digit, if we're at the hundreds digit go to the thousands and so on)
DBCE: 2A F2       BPL   $DBC2               ; if b>=0 then goto $DBC2, to add the carry flag to next digit(s)

; this part of the code checks to see if we get a new life or not.
DBD0: DC 46       LDD   $46                 ; read XYZ value of "bonus life every XYZ". 20000 decimal will be represented as D=$2000, 25000 will be represented as D=$2500
DBD2: 27 38       BEQ   $DC0C
DBD4: 31 04       LEAY  $0004,X             ; Y = X + 4
DBD6: EC 84       LDD   ,X                  ; read first 4 digits of score (from left to right)
...
DCF5: 22 03       BHI   $DCFA
DCF7: BD DD CE    JSR   $DDCE               ; draw object X at destination D
DCFA: AE 84       LDX   ,X
DCFC: 26 F3       BNE   $DCF1
DCFE: 39          RTS
...
; Here's the routine that draws and moves all the objects that are "omnidirectional" - that is to say they are not limited to 4 or 8
; degrees of movement like the other objects. Ever wondered how they glide so smoothly?
;
...
; In an omnidirectional object, these fields are the horizontal and vertical movement deltas (= values to be repeatedly added to X and Y coordinates of an object)
...
;
; the horizontal delta is added to the current X coordinate of the object ($000A,X) and the result (including fractional part) is written back to $000A,X.
...
DRAW_AND_MOVE_ALL_SPHEROIDS_ENFORCERS_SPARKS_V1:
DCFF: 96 59       LDA   $59
DD01: 85 08       BITA  #$08
DD03: 26 72       BNE   $DD77
...
DD34: 23 03       BLS   $DD39
DD36: BD DD CE    JSR   $DDCE               ; draw object X at destination D
...
DD5B: 22 71       BHI   $DDCE               ; draw object X at destination D
DD5D: 39          RTS

DD5E: 20 6E       BRA   $DDCE               ; draw object X at destination D



DRAW_ALL_OBJECTS_EXCEPT_PLAYER_V2:
DD60: 96 59       LDA   $59
DD62: 85 08       BITA  #$08
DD64: 26 10       BNE   $DD76
DD66: 9E 17       LDX   $17
...
DD6E: 23 02       BLS   $DD72
DD70: 8D 5C       BSR   $DDCE               ; draw object X at destination D
DD72: AE 84       LDX   ,X
DD74: 26 F4       BNE   $DD6A
DD76: 39          RTS
...
DRAW_ALL_OBJECTS_EXCEPT_PLAYER_V3:
DD77: 96 59       LDA   $59
DD79: 85 02       BITA  #$02
DD7B: 26 12       BNE   $DD8F
DD7D: 96 44       LDA   $44
...
DD89: 8D 43       BSR   $DDCE               ; draw object X at destination D
DD8B: AE 84       LDX   ,X
DD8D: 26 F8       BNE   $DD87
DD8F: 39          RTS
...
DRAW_AND_MOVE_ALL_SPHEROIDS_ENFORCERS_SPARKS_V2:
DD90: 96 59       LDA   $59
DD92: 85 08       BITA  #$08
DD94: 26 E1       BNE   $DD77
DD96: 9E 17       LDX   $17                 ; get pointer to all objects except player, into X
...
DDC5: 22 02       BHI   $DDC9
DDC7: 8D 05       BSR   $DDCE               ; draw object X at destination D
...
F115: AE 02       LDX   $0002,X              ; pointer to animation frame
...
F497: 10 8E D0 00 LDY   #$D000               ; address of function to jump to
...
F4F7: 86 04       LDA   #$04
F4F9: BD 5F 99    JSR   JMP_PRINT_STRING_LARGE_FONT    ; JMP $6147 print ALL ROMS OK
F4FC: C6 03       LDB   #$03
...
F67A: BD FA AF    JSR   $FAAF
F67D: BD D0 12    JSR   $D012
F680: 86 07       LDA   #$07
F682: 97 00       STA   $00
F684: BD FA AF    JSR   $FAAF
F687: 86 38       LDA   #$38
F689: 97 00       STA   $00
F68B: BD FA AF    JSR   $FAAF
F68E: 86 C0       LDA   #$C0
F690: 97 00       STA   $00
F692: BD FA AF    JSR   $FAAF
F695: 8D 16       BSR   $F6AD
F697: BD FA AF    JSR   $FAAF
F69A: 7E FA EE    JMP   $FAEE
...
F8AE: 8D 6D       BSR   $F91D                ; call routine to set RAM palette with A
F8B0: 8D 2E       BSR   $F8E0
F8B2: 8E F9 05    LDX   #$F905
F8B5: A6 80       LDA   ,X+
F8B7: DE 15       LDU   $15
F8B9: AF 49       STX   $0009,U
F8BB: 8D 60       BSR   $F91D                ; call routine to set RAM palette with A
F8BD: 86 80       LDA   #$80
...
F905: 02 03 04 10 18 20 40 80 00
...
;
; Display the switch that has been pressed in SWITCH TEST screen
;
...
FA29: 1C 01 95 1D 01 9C 00 00 00 00 00 00 00 00 00 00
...
FAD0: 27 08       BEQ   $FADA
FAD2: 86 02       LDA   #$02
FAD4: 8E FA CB    LDX   #$FACB               ; address of function to call
...
FBA2: 34 04       PSHS  B
FBA4: BD D0 A2    JSR   PACK_2_BYTES_INTO_A1
FBA7: 1F 89       TFR   A,B
FBA9: 84 0F       ANDA  #$0F
FBAB: 81 09       CMPA  #$09
FBAD: 23 02       BLS   $FBB1
...
FC9D: 86 3F       LDA   #$3F                 ; bits to send to sound board
...
FCCB: 86 09       LDA   #$09
FCCD: BD 5F 99    JSR   JMP_PRINT_STRING_LARGE_FONT    ; JMP $6147  - prints SOUND LINE 1
FCD0: 86 40       LDA   #$40
...
FCDF: 25 04       BCS   $FCE5                ; if carry flag is set,
...
CHECK_WORD_AGAIN:
FE0D: 30 1E       LEAX  $FFFE,X              ; X-=2. Now X points to the WORD that didn't hold the correct value
FE0F: A8 84       EORA  ,X
...
; Looks like this is a wait/delay function - because it does nothing else except repeat a countdown from #$5800 to 0 a specified number of times.
;
;
...
FE82: 30 1F       LEAX  $FFFF,X              ; decrement X by 1
FE84: C6 39       LDB   #$39
